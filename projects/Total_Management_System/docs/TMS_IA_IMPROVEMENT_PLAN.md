...
### 남은 라운드(구조 대공사 — 각 별도 회차 권장, 시각/구동 검증 큼)
- **P1-3 상세 패널 단일화**: `/[id]` 풀페이지를 패널 재사용 래퍼로(주문·판매·매입·제품). 진입경로별 UI 불일치 해소. ⚠️ 각 상세 회귀검증 필요
- **P1-5 DataGrid 수렴**: sales `SalesGridTable`·inventory `InventoryTable` → DataGrid. ⚠️ 목록 렌더 회귀
- **P3 PC 마스터-디테일**: events·serials·톡상담 좌목록+우패널. 신규 레이아웃
- **P2-3 발송 스텝화**: 판매 상세 발송 버튼 6개 적층 → 스텝형
- **P4 나머지**: window.confirm→ConfirmModal, 네비 그룹핑/B2B 명칭 통일, 로딩 Skeleton 통일
