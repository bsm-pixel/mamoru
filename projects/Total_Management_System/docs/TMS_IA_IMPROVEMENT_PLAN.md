...
### 남은 라운드 — ⚠️ 시각/구동 검증이 커서 사장님 확인 하에 진행 권장 (blind 자율변경 보류)
- **P1-5 DataGrid 수렴**: sales `SalesGridTable`·inventory `InventoryTable` → DataGrid. ⚠️ **실사용 대형 목록 리렌더 → 시각 QA 필수**
- **P2-3 발송 스텝화**: 판매 상세 발송 버튼 6개 적층 → 스텝형. ⚠️ **2026-05-26 사장님 채택 판매 디자인 영역 → Fix Preservation, 합의 후**
- **P4 window.confirm/alert(20+) → ConfirmModal/toast**: ⚠️ **파괴적 액션(삭제·취소) 플로우라 건별 실측 필요**
- **P3-1 events/serials PC 마스터-디테일**: 신규 좌목록+우패널 레이아웃
- **orders/[id] 단일화**: OrderActionBar 액션 패리티(cancel_pending 자동 ALPS 체크) 확인 후
- **소품목**: P1-6 금액 포맷(외국통화 혼재 주의), P3-3 전체상담 밀도, P3-4 세금계산서 좌우, 네비 그룹핑/B2B 명칭 통일
