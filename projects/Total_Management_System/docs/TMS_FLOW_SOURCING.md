...
```
STEP 1 매입(품목 입력)
  소싱 회차 생성 → 품목마다: 회사명·회사링크 / 📷제품이미지(1688 붙여넣기/업로드) / 품목링크 / 품목명·단가·MOQ / 특징
  · 이미지 = 주문 시 1688 이미지 Ctrl+V → 서버 영구저장(inbound_photos). 도착 후 촬영 불필요.
...
STEP 2 라벨 인쇄
  ...
  → KM-106D 등 라벨프린터로 출력 → 샘플에 부착
        ↓ (중국에서 물건 도착)
STEP 입고
  도착품에 라벨만 맞춰 붙임 = 물리 매칭. (이미지 이미 있음 → 촬영 불필요)
  ※ (선택) 라벨 QR 스캔(폰/PC) → /sourcing/inbound/{itemId} → 실물 사진·메모 추가 가능. 필수 아님.
        ↓
STEP 3 선별
  STEP1 제품 이미지 보고 채택/탈락 · 업체별 채택 현황("A 3/3 · C 2/5")으로 좋은 공장 선정
        ↓
STEP 4 선별 리스트 출력
...
| 항목 | 왜 없나 |
|------|---------|
| 자동 products INSERT | 등록은 사장님 직접. **단 옵트인(수동) 등록·연결은 2026-06-01 추가**(아래). 자동 INSERT는 여전히 없음 |
| SKU 자동채번·판매가 | 제품이 태어나는 아임웹에서 할 일 (제품 등록 모달에선 카테고리+next-sku 사용) |
| 발주 수량/소계 | 현 단계 역할 = 수량 아닌 "선별" |
| 단일 매입처 헤더 | 샘플은 제품마다 다른 상점 → 품목별 회사정보로 |

## 2026-06-01 추가

- **제품 이미지 = 주문 시 1688 이미지 붙여넣기(Ctrl+V)/업로드** → Storage 영구저장(inbound_photos). 도착품은 라벨만 붙여 매칭 → 일일이 촬영 불필요(폰/PC 입고매칭 사진은 선택).
- **라벨 = QR + 번호 + 품목명** (가격 제거). 가격은 **상단 [가격 리스트(A4)]** 로 별도 출력(번호·품목명·업체·¥단가·₩가격). 인쇄 공용 모듈 `lib/sourcing/label-print.tsx`(`printSourcingLabels`/`printSourcingPriceList`).
- **STEP1 품목별 [라벨인쇄] 칩**(복제 왼쪽): 팝오버(사이즈+수량) → 그 품목만 즉시 인쇄.
- **소싱↔제품/부자재 등록·연결** (마이그 100 `linked_product_id`): STEP4 선별리스트 [제품등록]/[부자재등록]/[기존 연결]. 부자재=products(category=SUP)라 FK 하나 공용. **연결=표시만**(제품 원본 비파괴). 제품/부자재 상세에 "소싱 정보(1688)" 카드(`GET /api/sourcing/by-product`). 모달 `_components/register-link-modals.tsx`. 중복 자동감지=v2.
