...
### 컴포넌트 (10+개)
- 매장방문 리스트 (2탭: 확정/지난내역) — 완료/취소 행 시각 구분
- 출장요청 리스트 (4탭: 오늘출장/대응필요/확정/지난내역) + 카카오맵 양방향 연동 + 지도 모니터뷰
  - 대응필요 탭: 가능요일/선호시간대 칩 표시 + 지역별 보기 토글 (address_sigungu 그룹핑)
  - 오늘출장 탭: MobileFieldDayView 재활용 (시간순 정렬 + 카카오네비 + PC 2컬럼 그리드)
- 톡상담 리스트 (2탭: 대응필요/지난내역) — 완료/취소 행 시각 구분
- 일정 달력 (초록: 매장, 보라: 출장)
...
### 03-26 — IA 개선 + 지도 모니터뷰 + 일정제안 최적화
| 항목 | 상태 |
|------|------|
| 출장요청 4탭 확장 (오늘출장/대응필요/확정/지난내역) | ✅ |
| 완료/취소 행 시각 구분 (3개 리스트 공통) | ✅ |
| 지도 모니터뷰 (마커→오버레이→상세 패널 연결) | ✅ |
| 가능요일/선호시간대 칩 표시 (gas_raw 파싱) | ✅ |
| SuggestTimeModal 연결 + 고객 가능 정보 참고 영역 | ✅ |
| 대응필요 탭 지역별 보기 토글 | ✅ |
