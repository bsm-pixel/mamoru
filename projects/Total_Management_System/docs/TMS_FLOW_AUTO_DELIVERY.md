...
## 🖥 화면 스텝바 = 같은 코드표 (2026-09-26)

배송 추적 스텝바(`components/orders/delivery-tracker.tsx` — 판매·주문·복원수리 3곳 공용)가 **옛 코드**로 굳어 있었다.

| | 고치기 전 | 고친 뒤 (실측 기준) |
|---|---|---|
| 1단계 | 접수 `01` — 응답에 없는 코드, 영원히 꺼짐 | **송장 생성** `02`(출력)·`12`(운송장등록) |
| 2단계 | 집하 `02`·`41` — **송장만 출력해도 켜짐**(사장님 지적) + 배달완료가 섞임 | **집하** `10` (기사 수거 스캔) |
| 3단계 | 배달중 `42`·`44` — 없는 코드 | **배달중** `20`·`21`·`40` |
| 4단계 | 배달완료 `91` — 없는 코드(state 보정으로만 켜짐) | **배달완료** `41`·`45` |

- 코드표 SSOT: **`lib/lotte/track-steps.ts`** (화면 전용). 집하 판정(자동 출고완료·알림톡)은 `alps-client.isPickedUpCode` 가 계속 담당 — **둘의 기준을 갈라놓지 말 것**(`12`는 양쪽 다 집하로 보지 않음).
- 마지막 스캔 줄도 없는 필드(`statDt`·`statTm`·`orgNm`)를 읽고 있어 빈 줄이었다 → 실제 필드(`scanYmd`·`scanTme`·`brnshpNm`·`status`)로 교체.
- 스텝바는 TMS 공용 `StatusStepper`(완료 ✓ 초록 / 현재 검정 / 이후 회색)로 통일 — 배송만 다른 모양(테라코타 원)이던 것을 정리.
...
## 🆕 롯데 밖으로 나가는 경로 (150, 2026-09-15)

**문제**: 시스템 전체가 "송장번호 = 롯데 ALPS 송장" 을 암묵 전제로 깔고 있었다.
롯데 송장을 뽑았다가 **ALPS 에서 집하취소하고 우체국으로 직접 보낸 건**(한선옥 DL-20260911-001)을 기록할 방법이 없었다.

| 그때 할 수 있던 것 | 실제로 벌어지는 일 |
|---|---|
| 송장 비우고 [출고 완료] | `body.tracking_number \|\| dl.tracking_number` 때문에 **죽은 롯데 송장이 그대로 남는다** |
| 우체국 송장 넣고 [출고 완료] | `deliveries` 에 택배사 컬럼이 없어 **롯데 송장으로 오인** → 크론이 매시간 헛조회, 배송완료 영영 안 뜸 |
| [송장 재발급] | 기존 송장을 취소하지 않고 롯데에 **또 접수** (유령 송장) |
| B2C 판매 | [택배 발송(롯데)] / [고객 수령 완료] 둘뿐 — 타 택배사 기록 경로가 **아예 없었다** |

**이제**: "롯데 ALPS 접수" 와 "실제로 어떻게 보냈는가" 를 분리한다.

- 택배사 SSOT = `lib/shipping/couriers.ts` — `isAlpsTrackable(courier_name)` **하나로만** 자동추적 대상을 판정한다.
  - `NULL`/빈값 = **true**(구데이터 호환 — 이 컬럼 생기기 전 송장은 전부 롯데였다). 여기를 false 로 바꾸면 기존 건이 통째로 추적에서 빠진다.
  - `'롯데택배'` = true / 그 외(우체국·CJ·한진·로젠·경동) = false / `'직접전달'` = 송장 없는 출고
- 크론 6개 블록 전부 루프 첫 줄에서 `if (!isAlpsTrackable(...)) continue;` — 헛조회 + "영영 안 끝나는 폴링"이 limit 50 을 점유하는 적체 차단
  ([1-A]/[1] 아임웹 orders 는 제외 — `courier_name` 을 아무도 롯데 외 값으로 쓰지 않는다. 아임웹 sync 는 `courier_code` 만 채운다)
- 화면: 롯데가 아니면 `DeliveryTracker`·자동감지 안내문을 숨기고 "직접 눌러 관리" 로 바꾼다

### 송장 취소 = **ALPS 에서 먼저, TMS 는 그다음**

🔴 **ALPS 취소 API 는 호출하지 않는다.** 실제로 성공한 적이 없다(사장님 실측 2026-09-15).
올바른 순서는 **① 사장님이 롯데 ALPS 에서 집하취소 → ② TMS [송장 취소] 로 기록 정리** 이고, 확인 모달이 그 순서를 안내한다.

- 납품 [송장 취소]: `PATCH action='cancel_shipment'` — 송장·택배사·출고흔적을 지우고 **status 를 confirmed 로 되돌린다**. 재고·매출·미수금 불변
- ⚠️ 판매·복원수리·빠른송장의 기존 [송장 취소]는 **여전히 ALPS 취소를 시도한다**(실패 시 경고 토스트). 동작 결과는 같아서 이번엔 건드리지 않았다 — 통일하려면 별건
- 잘 되는 **반품 수거접수**(`bookReturnPickup`, `ustRtgSctCd='02'`, 예약 API)와는 **완전히 다른 API**(`LOTTE_CANCEL_API_URL`, `canCd='01'`)다. 서로 영향 없음

### 화면 경로

| 대상 | 다른 택배사로 발송 | 직접 전달 | 송장 취소 |
|---|---|---|---|
| B2B 납품 | 액션 [실제로 보낸 방법] 택배사 선택 + 송장번호 → [출고 완료] | 택배사에서 `직접 전달` 선택(송장 자동 제거) | ✅ 150 신규 |
| B2C 판매 | 택배발송 섹션 [다른 택배사로 발송 (직접 입력)] → `POST /api/sales/[id]/ship {manual:true}` (ALPS 미호출) | 기존 [고객 수령 완료] | 기존(확인 모달 추가) |
| 복원수리 | 합포장 출고 모달의 직접 입력(택배사+송장) — 이미 있었음 | 송장 없이 상태 전환 | 기존 |
...
| **롯데 외 택배사** (우체국·CJ·한진·로젠·경동) | 롯데 집하취소 후 다른 택배사로 직접 발송 | `courier_name` 이 롯데가 아님 → `isAlpsTrackable` false 로 제외. 출고완료·배송완료 수동 (150) |
| **직접 전달** (`courier_name='직접전달'`) | 택배 없이 전달 | 송장번호 제거 → 자동 추적 자연 제외 (150) |
...
⚠️ 150 추가: `courier_name` 도 함께 봐야 한다. 새 테이블도 크론 루프 첫 줄에 `if (!isAlpsTrackable(row.courier_name)) continue;` 를 넣는다.
