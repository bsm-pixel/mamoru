...
> 최종 수정: 2026-09-26 — 배송 단계 표시 정정 · 송장 할 일 구글 Tasks 연동

---

## ✅ 완료 (09-26): 배송 추적 스텝바 정정 + 구글 할 일 연동 마무리
- **스텝바 코드가 통째로 옛것**: `01·42·44·91` 은 응답에 없는 코드 → 영원히 꺼짐. `02`(송장 출력)가 '집하'에 묶여 **기사 수거 전인데 완료처럼 보임**(사장님 지적). 실측 12건으로 코드표 확정 → 송장 생성/집하/배달중/배달완료 재정의 (커밋 bd6c9c8e)
  · 덤: 마지막 스캔 줄이 없는 필드를 읽어 빈 줄이던 것 → 실제 필드로. 자체 스텝바 → 공용 StatusStepper 통일
- **송장 미생성 → 구글 할 일(Tasks)**: 캘린더 종일일정은 체크가 안 돼 폐기하고 Tasks 로 전환. 등록 다음날 08시 생성 / 송장 생성되면 1시간 내 자동 삭제
- **막혔던 원인 2개**: ① 스코프 비교 오탐(email↔userinfo.email) ② **Tasks API 가 구글 클라우드에서 비활성**(프로젝트 652132380479) — 재연결로는 안 풀림
  · 이후 재발 방지: 설정 배너가 원인별로 안내 + 크론 푸시 1회/일 + `?probe=1`(읽기)·`?selftest=1`(쓰기) 자가진단 (커밋 4b7c992b·7e586102)
- 구글 계정 **2단계 인증 ON** — OAuth·앱비밀번호 영향 없음 확인

## ✅ 완료 (09-24): 작업 폴더 → 구글 드라이브 자동 미러
- 커밋마다 `scripts/mirror-to-drive.ps1`(.githooks/post-commit) → `C:\마모루드라이브\ClaudeWork\mamoru` (1,455개 164MB, .git·node_modules·.next 제외)

---

## ✅ 완료 (09-22): 출장 지연 안내 버튼 복원 + 알림톡 변수 수정 (커밋 884c374e)
- 원인① R1~R7 리모델(a29b989)에서 지연안내 UI 누락 — API·훅만 남아 호출처 0
- 원인② API 가 `time` 만 보내 솔라피 `#{visit_time}` 누락 → 문자 대체될 구조
- 공용 `field-delay-action.tsx` → 상세 패널(확정 출장) + 모바일 오늘출장 카드. 발송 실패는 502 로 표시
- 사장님 번호 실발송 ATA 4000 확인

## ✅ 완료 (09-21): 직접방문(당일수리) 달력 ↔ 수거 불가일 분리 (커밋 dd0cdb65 · 5e050c47)
- 수거 불가일(우체국)·주말·공휴일이 직접방문 날짜까지 막던 것(4eee8f6b) → 직접방문=상담 매장방문과 같은 매장 달력(휴무요일·휴무일)
- 복원수리 예약변경 페이지(직접방문 전용)의 수거 불가일 차단 제거 · 달력 제목 '방문 날짜 선택'
- ⚠️ 달력관리 휴무요일이 비어 있어 주말도 직접방문 열림(상담과 동일)

## ✅ 완료 (09-15): 관리자 푸시 전면 점검 — PC·모바일 동시 수신 + 아임웹 주문 실시간화 — 마이그 `151` (커밋 4280a0d9)
- 계기: 사장님 "PC·모바일 모두 알림이 안 온다 / 아임웹 주문은 오전 9시에만 들어온다"
- 근본① **사용자당 토큰 1개** 정책 → 새 기기에서 열 때마다 다른 기기 토큰 삭제(PC↔모바일이 서로를 끊음). 실측 등록기기 1대. 마이그 151 `device_id`로 **기기당 1개**로 전환, `api/push/cleanup-others` 삭제(재발 원인)
- 근본② 폴백 크론 `0 0 * * *`(한국 09시 1회) → 실측 지연 최대 **13시간**(08-21 11:01 주문 → 08-22 00:01 저장). `*/15`로 단축. 실시간 경로는 ORDER_CREATE 웹훅(09-14 연결 후 실주문 없어 **미검증**)
- 근본③ 웹훅 동기화 실패가 조용히 묻힘 → 즉시 경고 푸시 + 15분 크론 재시도
- 근본④ 푸시 누락: 매장방문 예약 **취소**·**일정변경**(고객 셀프), 아임웹 **무통장 입금완료**
- 설정에 「알림 받는 기기」 목록 신설(기기명·현재기기·개별 해제) + iOS PWA 안내. 전엔 등록 대수를 볼 방법이 없어 진단 자체가 불가능했음
- 문서 `docs/TMS_FLOW_PUSH.md` 신설(전 경로표 + 안 올 때 확인 순서)
- 남은 확인: 첫 실주문으로 웹훅 경로 검증(v2 getOrder 호환) — 실패 시 `⚠️ 아임웹 주문 동기화 실패` 푸시가 뜸

## ✅ 완료 (09-20): Make 시나리오 분류별 6개 재구축 + 옛 시나리오 7개 삭제

**계기** — 사장님 "상담 전체인데 왜 아임웹 주문 시나리오가 거기 들어가 있어?"
`상담접수_제작중` 한 시나리오에 상담·아임웹·EVENT가 **19분기** 뒤섞여 있었고, 분기 필터도
`{{1.template}}` / `{{1.event}}` / `_meta.func` 세 가지가 섞여 있어 추적이 불가능했다.

**새 구조** (필터 전부 `{{1.template}}` 로 통일)

| 시나리오 | id | 분기 | TMS 설정키 |
|---|---|---|---|
| `01 상담 (매장방문·출장·톡상담)` | 9837024 | 16 | `webhook_consultation` |
| `02 복원수리 (접수·상태변경·후기)` | 9837029 | 14 | `webhook_as_received` · `webhook_repair` |
| `03 판매·배송 (아임웹·오프라인 공통)` | 9837025 | 2 | `webhook_sales` |
| `04 반품·교환 (오프라인 판매분)` | 9837026 | 0 | `webhook_returns` (템플릿 대기 — 비움) |
| `05 아임웹 클레임 (취소·반품·교환)` | 9837018 | 5 | `webhook_imweb` |
| `06 EVENT` | 9837027 | 3 | `webhook_event` |

- **40분기 전부 사장님 번호로 실발송 테스트** → 전건 `type=ATA / statusCode=4000`(문자 대체 0건)
- 6개 전부 `metadata.scenario.dlq = true` (= Make UI 의 **Store incomplete executions**)
  — 09-17 솔라피 순단 1회로 시나리오가 꺼져 알림톡이 전면 중단된 사고의 재발 방지책.
  ⚠️ `05` 만 이 값이 빠져 있던 것을 09-20 발견·수정
- 09-17 승인된 템플릿 6종을 이때 연결: `IW-` 5종 + `출장 도착 지연 안내 / TMS`
  (= `field_delayed`. 그동안 TMS 버튼은 눌려도 Make 에 분기가 없어 **고객에게 안 나가고 있었다**)

**삭제한 옛 시나리오 7개** (blueprint 백업 후 삭제)
`상담접수_제작중` · `AS 접수 관련 발송` · `AS_상태변경 발송` · `EVENT관련(구축중` ·
`아임웹→솔라피 알림톡 자동발송` · `아임웹 토큰 발급(최초 1회)` · `아임웹 토큰 자동갱신(90분)`

- 삭제 전 확인: ① repo 내 Make 웹훅 하드코딩 0건(전부 `system_settings` 경유)
  ② 옛 시나리오 전부 마지막 실행이 전환(09-20 05:40) **이전**
  ③ 옛 분기 중 새 시나리오가 커버 못 하는 건 `찐막` EVENT 2분기뿐인데,
     `webhook_event` 가 다른 시나리오를 가리키고 있어 **이미 죽어 있던 분기**
- 🚨 **아임웹 토큰 시나리오 2개 삭제는 안전 조치**: 아임웹 `refresh_token` 은 1회용(rotation)이라
...
**✅ 09-21 오후 — 구축 완료.** 7건 승인 → TMS 코드 배포(37db9187, 교환 출고 2경로 + 아임웹 교환접수) → Make `04` 신설·`05`·`06` 교체 → `EVENT-신청완료` 도 의견 제출(변수 예시 2개) 후 승인·교체. **44경로 전부 최종 템플릿 · 재검수 0 · 실발송 ATA 4000.** 운영 문서는 노션 메뉴얼 「고객 알림톡 — 최종 구축 형태」 하나로 통합하고 알림톡 프로젝트·할일은 전부 닫음. 피그마 흐름도도 Make 6 시나리오 순서로 재편.

---

## ✅ 완료 (09-17): 알림톡 발송 주체 확정 + 아임웹 출고 구멍 메움 + 반품 용어 통일

**발송 주체 — 아임웹이 양식을 고정하는 구간만 아임웹에 맡긴다** (아임웹 공식 답변 근거)
> "기존 자동 발송되는 결제 완료·주문 완료·배송 관련 알림톡의 문구를 직접 만든 템플릿으로
>  교체하는 것은 어렵습니다 … 아임웹 제공 양식으로 발송됩니다.
>  다만 **CRM 캠페인에 해당 발송 조건이 제공되는 경우**에는 승인된 나만의 템플릿을 적용할 수 있어요."

| 구간 | 주체 | 근거 |
|---|---|---|
| 회원가입 | 아임웹 **CRM 캠페인** | 자체 템플릿 `JOIN` 승인됨(이미지·`{{이름}}` 포함) → **회원정보 API 승인 불필요해짐** |
| 입금 요청 · 결제완료 | 아임웹 기본 | 카드결제는 사장님이 그때그때 확인 못 함 → **누락 방지 > 문구 통제** |
| 배송중 · 취소 · 반품 · 교환 | TMS → Make → 솔라피 | 아임웹 양식 고정 + 집하 감지는 아임웹이 못 하는 타이밍 |

**🚨 발견 — 아임웹 주문 고객은 배송 안내를 한 통도 못 받고 있었다**
- 아임웹 「발송 안내」 = 꺼져 있음(09-17 관리자 화면 실측) + TMS 도 아임웹 주문엔 출고 알림톡 코드 없음
- 고객 경험: 결제완료 → **(침묵)** → 후기요청. 송장도 배송도 안내가 없었다
- 종전 노션의 "배송중은 아임웹 기본 알림톡이 보내니 끄지 말 것"은 **사실이 아니었다**(정정)
- 해결: 전용 템플릿(IW-배송중) 신설 대신 **오프라인과 같은 `sales_shipped` 재사용**
  → 템플릿 0개·이미지 0장 추가. 사장님 운영이 같으니 고객 문구도 같아야 한다(사장님 판단)
- 곁다리: 이 블록에만 `isAlpsTrackable` 가드가 빠져 롯데 외 송장도 ALPS 에 조회하고 있었다

**반품·교환 용어 통일 (마이그 154)**
- 같은 걸 가리키는 이름이 넷: `직접반납`·`고객반납`·`직접수거`·`방문수거`
  사장님: "뭔지 이해가 하나도 안되는 단어" — 게다가 `#{pickup_method}` 로 **고객에게 날것 그대로** 나가고 있었다
- → **`택배수거`(알림톡 O) / `대면수령`(알림톡 X)** 2가지. `방문수거`는 복원수리 용어와 충돌해 폐기
- `/returns` 에 「대면 수거완료」 버튼 · 접수·입고 알림톡 모두 택배수거 건만 발송
- 효과: "문 앞에 놓아주세요"가 **항상 사실**이 됨(전엔 직접 받는 건과 충돌) → 사장님이 만든 이미지 그대로 사용
- 반품/교환 구분: `returns.return_type` 이 있는데 알림톡에 안 넘기고 있었다 → `returnTypeLabel()` 로 전달
  (빈 문자열 절대 반환 안 함 — 변수가 비면 문자 대체 + 버튼 소멸)

**등록 대상 확정 10종** — 배너 완성 8장 + 교환 2종(기본형). 작업판에 본문 전부 준비
- ❌ 등록 안 함: `IW-결제완료`(아임웹 유지) · `IW-배송중`(sales_shipped 재사용) · `IW-배송완료`(구매후기 1통)
- 마이페이지 URL 확보: `https://mamoru.kr/shop_mypage` · 반품·교환 안내 페이지는 없어 WL 버튼 제거로 우회

**남은 것** — 노션 프로젝트 체크리스트 7단계에 "결제완료를 TMS 로 가져오기" 기록(사장님 희망, 반나절 작업)
...
- Pretendard는 Google Fonts 미제공 → jsDelivr CDN. 패밀리명 `Pretendard`(변수판 `Pretendard Variable`과 다름) 주의 명시
- 검증: 헤드리스 5페이지 로드 확인 + 변경 전후 레이아웃 비교(±3~6px, 최대 -82px는 줄바꿈 감소) + 라이브 curl 실측
- 한글 슬로건 **"가위를 만나는 즐거움"** 추가(영문=브랜드 선언 / 한글=고객 약속, 번역 아님). TMS `.tsx`는 제외(인쇄·캔버스)

## ✅ 완료 (09-15): 상세 패널 「액션」 5슬롯 전역 통일 — 5화면 (커밋 384ac7a6·2ea88a23·cf94953a·59ea187b·e5375150)
- 계기: 사장님 지적 "버튼명이라던지 이런 배치가 너무 난잡한데" → 납품부터 고치고 승인받아 전역 확산
- 근본: 액션이 늘 때마다 버튼을 아래로 쌓기만 해 **"지금 할 일"을 정하는 규칙이 없었다**. 그 결과 가장 큰 검은 버튼이 제일 안 쓰는 액션이거나(납품 [송장 재발급]), 파괴적 액션이 주 액션과 같은 무게였다(주문 빨간 full-width 2개, 반품 [취소]가 보조 버튼과 같은 줄)
- 표준 `components/ui/action-section.tsx` = ① 상태 한 줄 ② 주 액션(검정 1개) ③ 결제 ④ 접힘 ⑤ 파괴적 액션. 문서 `docs/TMS_UI_ACTION_PANEL.md`
- 적용: 납품 → 복원수리(카드 역할 분리: 비용·출고=정보 / 액션=별도) → 판매 → 주문 → 반품. 각 단계마다 검증·배포 분리
- 부수: 파랑·보라·주황 채움 버튼 제거(모노크롬 복귀) · `window.confirm` 3곳 → ConfirmModal · ④⑤가 문서/정보 섹션보다 위에 있던 순서 역전 2건(판매·주문) 수정
- 검증 방식 정립: 로그인 게이트 때문에 못 보던 화면을 **임시 devpreview 라우트 + 쿼리캐시 주입 + puppeteer 캡처**로 실제 렌더 확인 후 원복 (문서에 절차 기록)

## ✅ 완료 (09-15): 롯데 밖 배송 경로 — 다른 택배사·직접 전달 + 송장 취소 — 마이그 `150`
- 계기: 한선옥 납품(DL-20260911-001) 롯데 송장 발급 후 ALPS 집하취소 → 우체국으로 직접 발송. TMS에 기록할 방법이 없었음
- 근본: 시스템 전체가 "송장번호 = 롯데 송장" 암묵 전제. `deliveries`엔 `courier_name` 컬럼 자체가 없었고(150에서 추가), B2C 판매는 타 택배사 입력 경로가 아예 없어 [고객 수령 완료](매장수령)로 왜곡 기록할 수밖에 없었음
- SSOT `lib/shipping/couriers.ts` `isAlpsTrackable()` — NULL=롯데 간주(구데이터 호환). 크론 6블록 전부 가드 → 헛조회·영구 폴링 적체 차단
- 납품: [송장 취소](ALPS 호출 없음 — 사장님이 ALPS에서 집하취소 후 누르는 정리 버튼) + [실제로 보낸 방법] 택배사 선택·송장 입력 → 출고완료, `직접 전달` 선택 시 송장 제거
- 판매: [다른 택배사로 발송 (직접 입력)] 신규(`POST /ship {manual:true}`, ALPS 미호출) · 송장취소 확인 모달 + `courier_name` 정리 · 타 택배사면 `DeliveryTracker`·자동감지 안내 숨김
- 남은 정리(선택): 판매·복원수리·빠른송장의 기존 [송장 취소]는 여전히 동작 안 하는 ALPS 취소 API를 호출함(실패 경고 토스트). 납품과 통일하려면 별건

## ✅ 완료 (09-14): 알림톡 항상 발송 원칙 + 아임웹 취소·반품 알림톡 (TMS 발송) — 마이그 `148`·`149` (커밋 01fcf9ed·8ab01f80·52145023)
- **아임웹 웹훅 수신 기록기**: `/api/imweb/webhook` 모든 원본을 `imweb_webhook_events`(148)에 기록. 아임웹 웹훅 15개 전부 TMS로 연결(Make행 0). 테스트 보내기 10종으로 필드 실측(이름·전화 없음 · 품목 있음 · 거절 사유/환불금액 없음 · ORDER_REFUND 등록 불가)
- **취소·반품 알림톡** `lib/imweb/claim-notify.ts`: 취소접수·완료 / 반품접수·승인·완료 → Make(`notifications.webhook_imweb`, 폴백 없음=입력 시 가동). 거절 2종 = 고객 알림 없이 사장님 푸시. 관리자 직접처리 접수 생략 · 섹션 중복 방지 · 가짜주문 무발송
- **항상 발송 원칙(사장님 확정)**: 설정 알림톡 토글(전체·유형별 7·자동후기) + 화면 체크박스 4곳(상담 직접등록·일정변경·복원수리 입금확인·합포장) 삭제. 중복만 서버 규칙(무상 0원·현장결제 paid_on_site). 기존 38종 판정 대조 38/38 동일. 옛 설정값 17행 정리(149)
- 남은 외부 작업: 솔라피 IW- 5종 등록·검수 → Make 9794408 재구성·토큰 시나리오 삭제 → 설정에 URL 입력(가동) → 실주문 테스트 → 아임웹 기본 알림톡 해당 단계 OFF (노션 🛒 v2 카드 ⑧)
