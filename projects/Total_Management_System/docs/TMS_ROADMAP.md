...
### 전자문서 UI 구조 (6-6, 2026-03-01)
```
종이 계약서 형태 A4 레이아웃:
  헤더(MAMORU 구매계약서) → 계약자 정보(자동완성+직함+매장)
  → 필독/유의사항(법적 문구) → 제품 테이블(모달 선택)
  → 수령방법(발송/직접수령) → 결제방식(이체/카드/CMS+할부+선납/잔금)
  → 날짜 + 서명 2개(구매자/판매자) → 입금 계좌 안내

모바일/태블릿 전용: md:hidden (PC는 "태블릿에서 작성하세요" 안내)
```
