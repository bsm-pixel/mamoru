...
> 최종 수정: 2026-02-26 (R1~R7 전체 완료: 복원수리/상담/허브/주문/오프라인판매/전자계약서/시리얼넘버)
...
## Phase R (대규모 리모델) ✅ 전체 완료

> R1(복원수리) → R2(상담) → R3(허브) → R4(주문) → R5(오프라인판매+이카운트) → R6(계약서) → R7(시리얼)

| Phase | 내용 | 상태 |
|-------|------|------|
| R1 | 복원수리 대시보드 탭 바 리모델 + E2E 정상화 | ✅ |
| R2 | 상담관리 대시보드/탭 리모델 (지도 고정, 달력 색상, 양방향 연동) | ✅ |
| R3 | 허브 대시보드 리모델 (useHubStats 재작성, 카드 확장) | ✅ |
| R4 | 주문관리 온라인 강화 (결제칩, 메모란, 검색) | ✅ |
| R5 | 오프라인 판매 + 이카운트 ERP 연동 | ✅ |
| R6 | 전자 계약서 (서명 캔버스 + PDF + 알림톡) | ✅ |
| R7 | 시리얼넘버/바코드 (products 확장 + 바코드 스캔) | ✅ |
