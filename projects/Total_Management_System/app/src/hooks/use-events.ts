...
export function useCreateCampaign() {
  ...
  return useMutation({
    mutationFn: async (body: { name: string; type?: string; discount_rules?: DiscountRule[]; customer_notice?: string; payment_type?: string; items_label?: string }) => {
      const res = await fetch('/api/campaigns', {
...
export function useUpdateCampaign() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...body }: { id: string; name?: string; type?: string; status?: string; discount_rules?: DiscountRule[]; customer_notice?: string; payment_type?: string; items_label?: string }) => {
