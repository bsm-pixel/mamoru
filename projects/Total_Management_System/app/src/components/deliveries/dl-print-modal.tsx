...
export function DLPrintModal({ deliveryId, onClose }: Props) {
  ...
  // 부가세 계산
  const itemTotal = items.reduce((s, i) => s + (i.total_price as number), 0);
  const baseAmount = itemTotal - discountAmount;
  let supplyVal = 0, vatVal = 0, paymentVal = 0;
  if (vatType === 'separate') {
    supplyVal = baseAmount;
    vatVal = Math.round(baseAmount * 0.1);
    paymentVal = baseAmount + vatVal;
  } else if (vatType === 'none') {
    supplyVal = baseAmount;
    vatVal = 0;
    paymentVal = baseAmount;
  } else {
    supplyVal = Math.round(baseAmount / 1.1);
    vatVal = baseAmount - supplyVal;
    paymentVal = baseAmount;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 p-5">
          <div ref={printRef}>
            <h1>납 품 서</h1>
            <p className="subtitle">{dl.dl_number as string} · {dl.delivery_date as string}</p>
            ...
            <div className="info-grid" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
              ...
              <div className="info-box" style={{ width: '48%' }}>
                <h3 style={{ fontSize: '11px', fontWeight: 700, color: '#666', marginBottom: '8px', borderBottom: '1px solid #ddd', paddingBottom: '4px' }}>납품 정보</h3>
                <div style={{ fontSize: '11px', lineHeight: '1.8' }}>
                  <div><span style={{ color: '#888', display: 'inline-block', width: '50px' }}>납품일</span> {dl.delivery_date as string}</div>
                  <div><span style={{ color: '#888', display: 'inline-block', width: '50px' }}>증빙</span> {receiptLabel}</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '16px' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ background: '#f5f5f5', fontWeight: 600, fontSize: '11px', padding: '6px 8px', border: '1px solid #ddd' }}>품목</th>
            ...
            <div style={{ marginTop: '8px' }}>
              {discountAmount > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: '12px', color: '#e53e3e' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: '12px' }}>
                <span>공급가액</span><span>{formatKRW(supplyVal)}</span>
              </div>
              {vatType !== 'none' && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: '12px' }}>
                  <span>부가세 (10%)</span><span>{formatKRW(vatVal)}</span>
                </div>
              )}
