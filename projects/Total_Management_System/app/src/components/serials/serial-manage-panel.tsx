...
/**
 * 제품별 시리얼 관리 패널 (공통) — 일괄/단건 생성 · 상태/zone 변경 · 검색·필터.
 * `/products/[id]/serials` 와 `/serials`(제품별 관리 탭)에서 공유 (DRY, 2026-06-11 Phase 2).
 */
...
export function SerialManagePanel({ productId, productName }: Props) {
  ...
  // 일괄생성 폼 열 때 다음 시리얼 번호 미리보기 (MR{YY}{NNNNN})
  useEffect(() => {
  ...
  function selectAll() {
    setSelectedIds(new Set(serials.filter((s) => s.status === 'in_stock').map((s) => s.id)));
  }
  ...
  async function handleAddSerial() {
    ...
    await createSerial.mutateAsync({ product_id: productId, serial_number: newSerial.trim(), barcode: newBarcode.trim() || undefined });
    setNewSerial(''); setNewBarcode(''); setShowAddForm(false);
  }
  async function handleBatch() {
    if (batchCount < 1) return;
    await createBatch.mutateAsync({ product_id: productId, count: batchCount, lot_number: batchLot.trim() || undefined });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <Badge className="bg-neutral-100 text-neutral-600">총 {total}개</Badge>
        ...
        <Button variant="secondary" size="sm" onClick={() => { setShowBatchForm(!showBatchForm); setShowAddForm(false); }}>
          <Barcode size={14} />일괄 생성
        </Button>
        <div className="flex-1 relative min-w-[180px]">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" />
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="시리얼번호, 바코드, 고객명 검색"
      ...
      {/* 일괄 zone 변경 */}
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-2 p-3 bg-neutral-50 rounded-xl border border-neutral-200 flex-wrap">
          <span className="text-xs font-semibold text-neutral-600">{selectedIds.size}개 선택</span>
          <Button size="sm" variant="secondary" onClick={() => handleBulkZone('ready')} disabled={updateZone.isPending}>준비로 이동</Button>
          <Button size="sm" variant="ghost" onClick={() => handleBulkZone('display')} disabled={updateZone.isPending}>디스플레이로 이동</Button>
          <Button size="sm" variant="ghost" onClick={() => handleBulkZone('raw')} disabled={updateZone.isPending}>보관으로</Button>
          <Button size="sm" variant="ghost" onClick={() => setSelectedIds(new Set())}>선택 해제</Button>
        </div>
      )}
      ...
      <div className="flex gap-1 overflow-x-auto pb-1">
        {STATUS_TABS.map((tab) => (
          <button key={tab.value} onClick={() => setStatus(tab.value)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition ${status === tab.value ? 'bg-stone-900 text-white' : 'bg-white text-neutral-500 hover:bg-stone-50'}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <Card padding={false}>
        {isLoading ? (
          ...
          <div className="flex items-center justify-center h-32 text-sm text-neutral-400">시리얼 정보가 없습니다</div>
        ) : (
            ...
            {serials.map((serial) => (
              <div key={serial.id} className="flex items-center gap-3 px-4 py-2.5">
                {serial.status === 'in_stock' && (
                  <input type="checkbox" checked={selectedIds.has(serial.id)} onChange={() => toggleSelect(serial.id)} className="w-4 h-4 rounded border-neutral-300" />
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-mono font-semibold text-stone-900">{serial.serial_number}</span>
                    <Badge className={STATUS_COLOR[serial.status] || ''}>{STATUS_LABEL[serial.status] || serial.status}</Badge>
                    <Badge className={ZONE_COLOR[serial.warehouse_zone] || 'bg-neutral-100'}>{ZONE_LABEL[serial.warehouse_zone] || serial.warehouse_zone}</Badge>
                  </div>
                ...
                {serial.status === 'in_stock' && (
                  <select className="text-xs border border-neutral-200 rounded px-2 py-1 bg-white" value={serial.warehouse_zone}
                    onChange={(e) => { if (e.target.value && e.target.value !== serial.warehouse_zone) updateZone.mutate({ ids: [serial.id], warehouse_zone: e.target.value as 'raw' | 'ready' | 'display' }); }}>
                ...
                {serial.status === 'in_stock' && (
                  <select className="text-xs border border-neutral-200 rounded px-2 py-1 bg-white" value=""
                    onChange={(e) => { if (e.target.value) updateStatus.mutate({ id: serial.id, status: e.target.value }); }}>
