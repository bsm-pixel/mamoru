...
export function IntakeTab({ repairs, isLoading, onSelect }: IntakeTabProps) {
  ...
  const handleConfirm = (id: string) => {
    updateFields.mutate({ id, confirmed_at: new Date().toISOString() });
  };
  ...
  if (!repairs.length) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-neutral-400">
        ...
        <p className="text-sm">신규접수 건이 없습니다</p>
      </div>
    );
  }
        ...
        <Card key={r.id} className="hover:bg-neutral-50 transition cursor-pointer" onClick={() => onSelect?.(r.id)}>
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              {r.address && (
                <p className="text-xs text-neutral-500 truncate">{r.address} {r.address_detail}</p>
              )}
