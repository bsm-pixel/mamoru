-- 088: get_hub_stats — 접수시스템(A채널) 복원수리 매출을 실제청구액(total_amount) 안분으로 변경
--
-- 배경 (2026-05-19):
--   기존(080)은 A채널 매출 = qty_mamoru*10000 + qty_other*20000 (고정단가) 였음.
--   → 배송비·날변형 가공비 등 실제 청구액이 누락되어, 회계(/reports, total_amount 기준)와 불일치.
--   (예: 회계 접수시스템 95,000 vs 대시보드 마모루 60,000 — 차이 35,000 = 배송비+가공비 누락)
--
-- 해결: A채널 매출을 repairs.total_amount(= 수리비+배송비+가공비, 실제 청구액)로 변경.
--   마모루/타사 분리는 단가 비중(qty_mamoru*10000 : qty_other*20000)으로 안분 →
--   배송비·가공비 포함 + 분리합 = total_amount 정확 일치. 자루 수(count)는 기존대로 qty 합산(변동 없음).
--   use-dashboard-stats.ts(/repairs 화면) 안분 로직과 통일.
--
-- 080 대비 변경된 곳: monthRepairAmount(A) / monthRepairAOnly / monthRepairMamoru.amount(A) /
--   monthRepairOther.amount(A) — 4곳을 total_amount 안분으로. count 서브쿼리·B/C채널·배송비 제외 로직은 080 그대로.
    ...
    'repairs', (
      SELECT json_build_object(
        ...
        'monthRepairMamoru', json_build_object(
          'amount', COALESCE((
            SELECT SUM(CASE
              WHEN (qty_mamoru * price_mamoru + qty_other * price_other) > 0
                THEN ROUND(total_amount::numeric * (qty_mamoru * price_mamoru)::numeric / (qty_mamoru * price_mamoru + qty_other * price_other))
              WHEN total_amount > 0 THEN total_amount
              ELSE 0 END) FROM repairs
            WHERE created_at >= month_iso AND status != 'cancelled'
          ), 0) + COALESCE((
            SELECT SUM(osi.total_price) FROM offline_sale_items osi
            INNER JOIN offline_sales os ON os.id = osi.sale_id
        ...
        'monthRepairOther', json_build_object(
          'amount', COALESCE((
            SELECT SUM(CASE
              WHEN (qty_mamoru * price_mamoru + qty_other * price_other) > 0
                THEN total_amount - ROUND(total_amount::numeric * (qty_mamoru * price_mamoru)::numeric / (qty_mamoru * price_mamoru + qty_other * price_other))
              ELSE 0 END) FROM repairs
            WHERE created_at >= month_iso AND status != 'cancelled'
          ), 0) + COALESCE((
            SELECT SUM(osi.total_price) FROM offline_sale_items osi
