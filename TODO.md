...
> 최종 수정: 2026-04-06 (메인 리뉴얼+AS리포트 모노크롬화+딜러페이지 삭제+Brand Guide 100% 통일)
...
- [ ] 메인 페이지 아임웹 배치 확인 — top+상품위젯+btm 3단 정상 표시
- [ ] 메인 이미지 삽입 — Before/After, 컨설팅, 도구 카드 (촬영분)
- [ ] 대시보드 할일 메모 테스트 — 입력/체크/삭제 동작 확인
- [ ] B2B 거래처 상세 테스트 — 클릭 → 상세 → 수정
- [ ] 매입관리 제품 검색 리스트 테스트
- [ ] 준비표 인쇄 테스트 — 복수 선택 + 단건
- [ ] 남은 하드코딩 교체 (수리비, 저재고 기준, 체류 경고일 → 설정값)
- [ ] 거래명세서 사업자 정보 → 설정 연동 (ReceiptModal 구조 변경)
- [ ] 사이드바 커스텀 → 설정값 연동
