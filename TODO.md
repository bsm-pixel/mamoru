...
- [x] **1. 상담 페이지군** (2026-05-27 완료, `83eda88`) — 4탭(전체/매장/출장/톡), 전체 탭 기본값, 시안 B 톤 적용
- [x] **2. 복원수리 페이지군** (2026-05-27 완료, `5e508ff`) — 페이지 배경 stone-50, 매출 KPI A2 그라데이션, 상태 4카드 시안 B 톤, 6단계 탭 stone-900. **+ /sales 매출 카드 A2 동시 적용**
- [ ] **3. 주문 페이지군** (`/orders` 대시보드/리스트/송장/상세) ← 진행 중
...
### 상담 페이지군 시안 B 톤 통일 + 전체 탭 (2026-05-27 사장님 승인)

**채택안**: 시안 B (적극) + 전체 탭 추가 (기본값)

| 변경 | 커밋 | 비고 |
|---|---|---|
| 시안 B 톤 통일 + 전체 탭 + AllConsultationsList 신규 | `83eda88` | 4탭(전체/매장/출장/톡), 기본=전체 |
| 디자인 모니터 시안 A/B 비교 | `a2a1606` | 적용 후 § 삭제 |

**핵심 변화**:
- 4탭: [전체] [매장방문] [출장요청] [온라인상담] — 기본값=전체
- 신규 `all-consultations-list.tsx`: 활성 6상태 시간순 통합, 좌측 색 줄(emerald/violet/blue)
- 요약 3카드 → 대시보드 CategoryCard 톤 (3xl 숫자)
- 페이지 배경 stone-50, 탭 활성색 stone-900
- format.ts `change_requested` orange-100 → warning-soft 정규화
- 3개 리스트(store/field/talk) 서브탭 색 모노크롬화

**회계·데이터 안전성**: useConsultations / DashboardStats / 알림톡 / Calendar 무수정
...
## ✅ 완료 (05-19): 복원수리 버그 2건 fix + 매출 집계 정합

### 1. 수거접수필요 탭 orphan fix (`26e39bb`)
- 증상: 방문수거 건 "접수 확인" 누르면 리스트에서 사라짐 (대시보드 카운트는 정상)
- 원인: 수거접수필요 탭 필터가 `confirmed_at IS NULL`(접수확인 전) → 접수확인 후 3개 탭 모두 탈락 = orphan
- fix: `confirmed_at IS NOT NULL` 1줄. 데이터 변경 0, 기존 건 자동 복구
- 정상 흐름: 신규접수 → 접수확인 → [방문수거] 수거접수필요 → 수거접수완료 → 입고대기 / [직접발송] 입고대기

### 2. 복원수리 A채널 매출 실제청구액 안분 (`6371801` + 088 RPC)
- 증상: 회계(649,000) vs 대시보드(744,000) 복원수리 매출 불일치
- 원인: 대시보드 A채널 = qty×고정단가(배송비·가공비 누락), 회계 = total_amount(실제청구액)
- fix: 대시보드 A채널도 `total_amount` 안분 (마모루/타사 단가비중 분리, 분리합=total_amount 정확)
  - use-dashboard-stats.ts(/repairs) + 088_hub_stats RPC(/dashboard 허브) — 사장님 SQL 실행 완료
- 결과: 금액 계산식 통일 (B2B 464,000 완전 일치). 대시보드 744,000(고정단가 추정) → 672,000(실제청구액)
- **시점은 목적별 유지** (사장님 결정): 회계=입금완료(재무) / 대시보드=접수발생(운영). 남은 23,000 차이 = 미입금/시점차로 정상
- 흐름도 [TMS_FLOW_REPAIR](projects/Total_Management_System/docs/TMS_FLOW_REPAIR.md) "매출 집계 기준" 박제
...
## ✅ 완료 — 출장 상담 알림톡 흐름 fix 묶음 (2026-05-13~14, 배포 완료)

GAS→TMS 이식 시 누락된 변수가 출장 흐름 곳곳에서 깨져 있어 일괄 복구. DB·Make·솔라피 변경 없음.
- 출장 확정 알림톡(`field_confirmed`)이 SMS로 떨어지던 버그 — `address`/`change_request_link` 추가 (`209ef65`)
- 수동 일정 변경(`field_rescheduled`) 알림톡 미발송 + 버튼/모달 라벨 동적화 ("수동 일정 확정" ↔ "수동 일정 변경") (`a215a09`)
- 일정변경 요청 페이지 404 ("예약 정보를 찾을 수 없습니다") — `resched` route 가 `uid` 도 받게 + page 가 `?uid=` 로 호출 (`f083f43`)
- 출장 취소 알림톡(`field_cancelled`) 본문에 `#{visit_date}` 등 원본 변수 그대로 발송되던 버그 — `visit_date`/`visit_time`/`address` 추가 (`4a9cd78`)
- 흐름도(TMS_FLOW_CONSULTATION) + 매뉴얼(MANUAL_CONSULTATION) + ROADMAP 갱신

## ✅ 완료 — TMS 매입(발주→입고) 흐름 보강 (2026-05-13, 배포·SQL 완료)

가위 = 제작품이라 주문≠입고 흔함 + 업체 발송 후 잔금만 먼저 보내는 경우 대응 → 결제/입고 독립화.
- 입고 전 "잔금 지불 처리" 버튼 (발주완료/선납완료) — `balance_paid_at` 기록 (`bc03567` + UI 패널 누락 fix `5e5c901`)
- "입고 확인" → 입고검수 모달 (품목별 주문/실수령 편집 + 재계산 프리뷰), 재고는 실수령 기준, `purchase_order_items.received_quantity` 기록, 마이그레이션 081 (`09af5e7`)
- 입고 전 발주 품목 수정 허용 (작성중/발주완료/선납완료 + 잔금 미지불) → 총액·잔금 자동 재계산 (`6056a63`)
- 흐름도(TMS_FLOW_INVENTORY) + 매뉴얼(MANUAL_INVENTORY) + ROADMAP 갱신. 마이그레이션 081 사장님 SQL 실행 완료.

## ✅ 완료 — TMS 매출 3분할 (2026-05-12~13, 배포·SQL 완료)

총매출 = B2C 제품 + B2B 제품(딜러·아카데미·납품) + 복원수리 전체(A 접수 + B 판매RS + C 납품RS). RS는 제품 매출에서 제외.
- 2-A~2-E + 3-A: useHubStats salesB2C/salesB2B / RPC 078 / 대시보드 KPI 3분할 / 회계 리포트 RS 집계 정확화(by_product RS 제외) / customers.default_repair_price + 거래처 단가 자동채움 / 판매입력 "복원수리" 모드
- 후속 fix(`4f67218`): 회계 "이번 달" 프리셋 UTC 버그(4/30→5/1) / B2B 두 줄 통합 / 판매입력에서 RS 더미 제품·카테고리 숨김 / 복원수리 모드 배송비 3,000원 토글 / 납품 "+B2B수리" 추가항목 category='RS'
- 배송비 자루 fix(`4de93f2` + RPC 080): 배송비 = 복원수리 매출 포함, "자루 수"에서만 제외
- 마이그레이션 078·079·080 사장님 SQL Editor 실행 완료. 커밋 `5c96d1b`~`4de93f2`. 상세: `memory/project_tms_repair_revenue_split.md`
- (선택, 미진행) 납품 페이지 "이번달 매출"을 회계 B2B 제품과 동일 기준으로 정렬 / 기존 RS 더미 제품 is_active=false — 필요 시
