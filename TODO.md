...
> 최종 수정: 2026-04-17 (외화 발주 + 수금 처리 + 제품 그룹핑 정합성 + 납품서 편집 + 이미지 복사)
> **미완료 항목을 상단에, 완료 이력을 하단에 배치**

---

## 📌 네이버 리뷰 추출 (다음 작업 시 진행)

### 사전 준비
1. 크롬에서 **네이버 스마트플레이스 관리자** 로그인
2. **리뷰 관리** 페이지로 이동
3. **스크롤 끝까지 내리기** — 모든 리뷰가 화면에 로드될 때까지 (더보기 있으면 모두 클릭)

### 추출 실행
4. **F12** 키 누르기 (개발자도구 열림)
5. **Console** 탭 클릭
6. `projects/marketing/naver_review_extract.js` 파일 내용을 **전체 복사** (VS Code에서 Ctrl+A → Ctrl+C)
   상세 가이드: [projects/marketing/README.md](projects/marketing/README.md)
7. Console 입력창에 **붙여넣기** (Ctrl+V) → **Enter**
8. 자동으로 `naver_reviews.json` + 이미지 파일들이 다운로드됨

### 결과 확인
- **naver_reviews.json**: 리뷰 전체 데이터 (작성자/날짜/본문/태그/별점/이미지URL)
- **review_N_photo_N.jpg**: 리뷰 이미지 개별 파일
- 다운로드 폴더에서 확인 → TMS 리뷰 등록 시 참조

> ⚠️ Console에 붙여넣을 때 "붙여넣기를 허용하시겠습니까?" 경고 뜨면 `allow pasting` 입력 후 Enter, 그다음 다시 붙여넣기
