...
def build_children(rows):
    ...
    ch.append(_table(
        ...
        [
            ...
            # 2026-09-21 인스타 SEO 반영 (claude/28): 첫 줄=구글 검색 제목 역할 · 해시태그 최대 5개(인스타 2025-12 정책)
            ["인스타", "캡션 첫 줄 (125자 안)", "💎 1개 (블로그 메인과 같게)", "메인 검색어로 시작 · 질문 또는 반전 한 줄 · 낚시 ❌", f"{main_kw}, 비싼 게 답일까?"],
            ["인스타", "해시태그 최대 5개", "고정 2 + 💎·🛠 표에서 3개", "#마모루 #미용가위 + 주제 3개 · 5개 넘기지 않기",
             " ".join(["#마모루", "#미용가위"] + [f"#{k}" for k in dict.fromkeys(x.replace(' ', '') for x in subs + [r['kw'] for r in repair]) if k not in ('마모루', '미용가위')][:3])],
            ["인스타", "사진 대체 텍스트", "💎 1개", "사진마다 '보이는 것 + 검색어' 한 문장 (고급 설정)", f"{main_kw} 클로즈업 사진 · 보이는 그대로 한 문장"],
            ["인스타 릴스 · 캐러셀 표지", "첫 1초 화면 텍스트 · 표지 헤드라인", "💎 1개", "검색어 그대로 짧게", main_kw],
