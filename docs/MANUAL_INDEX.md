...
`TMS_FLOW_SALES` · `TMS_FLOW_REPAIR` · `TMS_FLOW_CONSULTATION` · `TMS_FLOW_EVENT` · `TMS_FLOW_ORDERS`(아임웹 주문·배송대기 3단계·아임웹 배송상태 자동 역동기) · `TMS_FLOW_REVIEW`(후기 요청→작성→저장→상품상세 노출) · `TMS_FLOW_REVIEW_EVENT`(리뷰 이벤트/당첨자) · `TMS_FLOW_CUSTOMERS` · `TMS_FLOW_INVENTORY` · `TMS_FLOW_SERIAL` · `TMS_FLOW_SOURCING` · `TMS_FLOW_ACCOUNTING` · `TMS_FLOW_AUTO_DELIVERY`(배송추적·집하) · `TMS_FLOW_IMWEB_BANNER`
